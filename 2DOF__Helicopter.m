...
load('2dof_helicopter.mat'); % Carrega os dados de entrada
ui = u(:,1:end)';
yi = y(:,1:end)';
clear u y
passo = 10;
tk = (1:length(ui(:,1)))'; % Vetor de amostras
...
max_u1 = 17; min_u1 = 12; % Valor maximo e minimo da entrada para normalizacao
max_u2 = 1; min_u2 = -15; % Valor maximo e minimo da entrada para normalizacao
max_y2 = 281; min_y2 = -319; % Valor maximo e minimo da saida para normalizacao
max_y1 = 29; min_y1 = -41; % Valor maximo e minimo da saida para normalizacao
u1_n = (u(:,1)-min_u1)/(max_u1-min_u1); % normalizando
u2_n = (u(:,2)-min_u2)/(max_u2-min_u2); % normalizando
y1_n = (y(:,1)-min_y1)/(max_y1-min_y1); % normalizando
y2_n = (y(:,2)-min_y2)/(max_y2-min_y2); % normalizando
%% Inicializa as variaveis do Algoritmo ETS
% Parâmetros do algoritmo
r = 0.75; % raio de influencia de um cluster
epsilon = 0.3; % Fator de raio de cluster
m = 2.5; %Ponderamento exponencial para calculo de pertinencias
...
xn = [u1_n';u2_n';y1_n';y2_n']; % Vetor de dados normalizados
...
Ni = 150; % Instante de inicializacao
nent = length(u(1,:));
nsai = length(y(1,:));
p_pm = 2;
v = [u';y'];
...
erro_medio(Ni+1,:) = (y(Ni+1,:)-ye(Ni+1,:)).^2;
raiz_erro_medio(Ni+1,:) = sqrt(erro_medio(Ni+1,:));
j = 1;
for i = Ni+2:length(y(:,1))
    j = j + 1;
    erro_medio(i,:) =  ((j-1)*erro_medio(i-1,:) + (y(i,:)-ye(i,:)).^2)./j;
    raiz_erro_medio(i,:) = sqrt(erro_medio(i,:));
end

figure
subplot(2,1,1)
plot(tk,u(:,1),'b','LineWidth',1)
legend('Input 1')
legend('FontSize',5)
xlabel('Sample')
ylabel('Pitch motor voltage (V)')
grid on
xticks(0:250:1500)
subplot(2,1,2)
plot(tk,u(:,2),'b','LineWidth',1)
legend('Input 2')
legend('FontSize',5)
xlabel('Sample')
ylabel('Yaw motor voltage (V)')
grid on
xticks(0:250:1500)

figure
plot(tk,qt_reg,'b','LineWidth',1)
xlabel('Sample')
...
figure
subplot(2,1,1)
plot(tk(Ni+1:end),y(Ni+1:end,1)-ye(Ni+1:end,1),'b',tk(Ni+1:end),raiz_erro_medio(Ni+1:end,1),'r','LineWidth',1)
legend('Error - output 1','RMSE - output 1')
xlim([Ni 1500])
xlabel('Sample')
...
grid on
subplot(2,1,2)
plot(tk(Ni+1:end),y(Ni+1:end,2)-ye(Ni+1:end,2),'b',tk(Ni+1:end),raiz_erro_medio(Ni+1:end,2),'r','LineWidth',1)
legend('Error - output 2','RMSE - output 2')
xlim([Ni 1500])
xlabel('Sample')
% xlabel('Tempo (s)','Interpreter','latex')
ylabel('Yaw angle (°)')
...
% xc(2,:) = xcn{end}(2,:)*(max_y-min_y)+min_y;
    figure
    subplot(2,1,1)
    plot(tk,y(:,1),'k',tk,ye(:,1),'b--','LineWidth',1)
    xlabel('Sample')
ylabel('Pitch angle (°)')
legend('Output 1','Estimated output 1')
grid on
xticks(0:250:1500)
subplot(2,1,2)
    plot(tk,y(:,2),'k',tk,ye(:,2),'b--','LineWidth',1)
    xlabel('Sample')
ylabel('Yaw angle (°)')
legend('Output 2','Estimated output 2')
grid on
xticks(0:250:1500)
...
ye = ye(Ni+1:end,:);
for j = 1:nsai
    RMSE(j) = sqrt(mean((y(:,j)-ye(:,j)).^2))
    NDEI(j) = RMSE(j)/std(ye(:,j))
    num = 0;
    den1 = 0;
    den2 = 0;
    for i = 1:length(y(:,j))
        num = num + (y(i,j)-mean(y(:,j)))*(ye(i,j)-mean(ye(:,j)));
        den1 = den1 + (y(i,j)-mean(y(:,j)))^2;
        den2 = den2 + (ye(i,j)-mean(ye(:,j)))^2;
    end
    CCR(j) = num/(sqrt(den1*den2))
    num = 0;
    den = 0;
    for i = 1:length(y(:,j))
        num = num + (y(i,j)-ye(i,j))^2;
        den = den + (y(i,j)-mean(y(:,j)))^2;
    end
    R2(j) = 1-num/den
end
...
% y2m = (-0.2:0.05:1.2)';
% figure
% plot3(xcn{end}(1,:),xcn{end}(2,:),xcn{end}(3,:),'b*','LineWidth',1)
% hold on
% [X1m,X2m,Y1m] = meshgrid(x1m,x2m,y1m);
% for l = 1:R
%     l
%     for i = 1:length(x1m)
%         for i2 = 1:length(x2m) 
%             for j = 1:length(y1m)
%                 dist = sqrt(([x1m(i);x2m(i2);y1m(j);xcn{end}(4,l)]-xcn{end}(:,l))'*det(F(:,:,l))^(1/n)*inv(F(:,:,l))*...
%                     ([x1m(i);x2m(i2);y1m(j);xcn{end}(4,l)]-xcn{end}(:,l)));
%                 Dm{l}(i,i2,j) = pex*(exp(dist)-1) + (1-pex)*dist;
%             end
%         end
%     end
%     for j = 1:length(y1m)
%         Dm{l}(:,:,j) = Dm{l}(:,:,j)';
%     end
%     p = patch(isosurface(X1m,X2m,Y1m,Dm{l}, epsilon*r),'LineWidth',1);
%     isonormals(X1m,X2m,Y1m,Dm{l},p);
%     p.FaceColor = 'red'; % Cor da superfície
%     p.EdgeColor = 'none'; % Oculta as bordas da superfície
%     p.FaceAlpha = 0.05; % Define a transparência (0 = totalmente transparente, 1 = opaco)
% end
% legend('Clusters centers','Isometric surfaces','FontSize',20)
% xlabel('Input 1','FontSize',20)
% ylabel('Input 2','FontSize',20)
% zlabel('Output 1','FontSize',20)
% clear Dm
% figure
% plot3(xcn{end}(1,:),xcn{end}(2,:),xcn{end}(4,:),'b*','LineWidth',1)
% hold on
% [X1m,X2m,Y2m] = meshgrid(x1m,x2m,y2m);
% for l = 1:R
%     l
%     for i = 1:length(x1m)
%         for i2 = 1:length(x2m) 
%             for j = 1:length(y2m)
%                 dist = sqrt(([x1m(i);x2m(i2);xcn{end}(3,l);y2m(j)]-xcn{end}(:,l))'*det(F(:,:,l))^(1/n)*inv(F(:,:,l))*...
%                     ([x1m(i);x2m(i2);xcn{end}(3,l);y2m(j)]-xcn{end}(:,l)));
%                 Dm{l}(i,i2,j) = pex*(exp(dist)-1) + (1-pex)*dist;
%             end
%         end
%     end
...
%         Dm{l}(:,:,j) = Dm{l}(:,:,j)';
%     end
%     p = patch(isosurface(X1m,X2m,Y2m,Dm{l}, epsilon*r),'LineWidth',1);
%     isonormals(X1m,X2m,Y2m,Dm{l},p);
%     p.FaceColor = 'red'; % Cor da superfície
%     p.EdgeColor = 'none'; % Oculta as bordas da superfície
%     p.FaceAlpha = 0.05; % Define a transparência (0 = totalmente transparente, 1 = opaco)
% end
...
% xlabel('Input 1','FontSize',20)
% ylabel('Input 2','FontSize',20)
% zlabel('Output 2','FontSize',20)
% figure
% regra = 5;
% for i = 1:n
%     for j = 1:n
%         for k = 1:N
%             if isempty(A_{regra,k})
%                 valorplotar(k) = 0;
%             else
%                 valorplotar(k) = A_{regra,k}(i,j);
%             end
%         end
%         plot(valorplotar)
%         hold on
