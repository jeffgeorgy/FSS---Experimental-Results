...
u = [u1,u2];
y = Fogtrein(:,3);
tk = (1:length(u1))'; % Vetor de amostras
%Amostras em regime permanente
max_u1 = 46; min_u1 = -1; % Valor maximo e minimo da entrada para normalizacao
max_u2 = 360; min_u2 = 0; % Valor maximo e minimo da entrada para normalizacao
max_y = 91; min_y = 30; % Valor maximo e minimo da saida para normalizacao
u1n = (u1-min_u1)/(max_u1-min_u1); % normalizando
u2n = (u2-min_u2)/(max_u2-min_u2); % normalizando
...
s = 0.85; % Fator de sensibilidade para determinacao de candidato
...
xn = [u1n';u2n';yn']; % Vetor de dados normalizados
x = [u1';u2';y']; % Vetor de dados original
...
yticks(0:1:4)
figure
plot(tk(Ni+1:end),1000*(y(Ni+1:end)-ye(Ni+1:end)),'b',tk(Ni+1:end),1000*raiz_erro_medio(Ni+1:end),'r','LineWidth',1)
legend('Error','RMSE')
xlim([Ni 4500])
xlabel('Sample')
