...
u_on = (u_o-min(u))/(max(u)-min(u)); % Normalizando as amostras ordenadas
y_on = (y_o-min(y))/(max(y)-min(y)); % Normalizando a saida
un = (u-min(u))/(max(u)-min(u)); % normalizando
yn = (y-min(y))/(max(y)-min(y)); % normalizando
u_n = (u_-min(u))/(max(u)-min(u)); % normalizando
y_n = (y_-min(y))/(max(y)-min(y)); % normalizando
...
figure
plot(u,y,'c*',u_o,y_o,'k--',u_o,y_o,'k*',xc{end}(1,:),xc{end}(2,:),'r*','LineWidth',1)
hold on
legend('Vectors of Data','Static Curve','Steady-State Data','Centers of Clusters','Interpreter','latex')
...
FIT = 1 - norm(y(Ni+1:end)-ye(Ni+1:end))/norm(y(Ni+1:end)-mean(ye(Ni+1:end))*ones(N-Ni,1))
VAF = 1 - var(y(Ni+1:end)-ye(Ni+1:end))/var(y(Ni+1:end))
